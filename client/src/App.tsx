...
/** Calculate compound heat score (normalized 0-100) */
function calcHeatScore(h: Hotspot): number {
  ...
  // Weighting formula: retweets are heaviest, followed by likes, then comments/replies
  const raw = likes * 2 + retweets * 3 + replies * 1.5 + comments * 1.5 + quotes * 2 + views / 100;
  // Log compression to 0-100
  if (raw <= 0) return 0;
  return Math.min(100, Math.round(Math.log10(raw + 1) * 25));
}

function getHeatLevel(score: number): { label: string; color: string } {
  if (score >= 80) return { label: 'Exploding', color: 'text-red-400' };
  if (score >= 60) return { label: 'Hot', color: 'text-orange-400' };
  if (score >= 40) return { label: 'Warm', color: 'text-amber-400' };
  if (score >= 20) return { label: 'Cool', color: 'text-blue-400' };
...
function App() {
  ...
  useEffect(() => {
    const unsubHotspot = onNewHotspot((hotspot) => {
      ...
      showToast('New hotspot discovered: ' + hotspot.title.slice(0, 30), 'success');
      loadData();
    });
  ...
  const handleSearch = async (e: React.FormEvent) => {
    ...
    try {
      ...
      showToast(`Found ${result.results.length} results`, 'success');
    } catch (error) {
  ...
  const handleManualCheck = async () => {
    ...
    try {
      ...
      showToast('Hotspot check triggered', 'success');
  ...
  // Toggle relevance reason expansion
  ...
  // Toggle original content expansion
  ...
  // Toggle all relevance reasons expansion
    ...
    <div className="min-h-screen bg-[#050510] relative overflow-hidden">
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-xs text-slate-500">AI Hotspot Radar</p>
              </div>
            </div>
      ...
      <main className="relative z-10 max-w-6xl mx-auto px-6 py-8">
        ...
        <div className="flex gap-2 mb-8">
          {([
            { key: 'dashboard', label: 'Hotspot Radar', icon: Activity },
          ...
          <div className="space-y-8">
            ...
            {stats && (
              <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                <motion.div 
                  initial={{ o                    <div className="flex items-center gap-2 text-slate-500 text-sm mb-2">
                ...
                >
                  ...
                  </div>nt-to-br from-cyan-500/5 to-transparent opacity-0 group-hover:opacity-100 transition-opacity" />
                  <div className="relative">
                    <div className="flex items-center gap-2 text-slate-500 text-sm mb-2">
                      <Clock className="w-4 h-4" />
                      今日新增
                ...
                >
                  ...
                  </div>ald-500/5 to-transparent opacity-0 group-hover:opacity-100 transition-opacity" />
                  <div className="relative">
                    <div className="flex items-center gap-2 text-slate-500 text-sm mb-2">
                      <Target className="w-4 h-4" />
                      监控词
            ...
            <div>
              <div className="flex items-center justify-between mb-4">
                <h2 className="text-lg font-semibold text-white flex items-center gap-2">
                  ...
                  Real-time Hotspot Feed
                </h2>
                <span className="text-xs text-slate-600">Auto updates every 30 mins</span>
              </div>
              ...
              {isLoading ? (
                ...
                <div className="text-center py-16 rounded-2xl border border-dashed border-white/10">
                  ...
                  <p className="text-slate-500">No hotspots found yet</p>
                  <p className="text-sm text-slate-600 mt-1">Add keywords to start tracking</p>
                </div>
              ) : (
                <div className="space-y-3">
                  {/* Toggle all reasons expansion */}
                  {hotspots.some(h => h.relevanceReason) && (
                    <div className="flex justify-end">
                      ...
                      >
                        ...
                        {allReasonsExpanded ? 'Collapse All Reasons' : 'Expand All Reasons'}
                      </button>
                    </div>
                  )}
                    ...
                    >
                      <div className="flex items-start justify-between gap-4">
                        <div className="flex-1 min-w-0">
                          ...
                          {hotspot.summary && (
                            <div className="mb-3">
                              <span className="text-[10px] text-blue-400/60 font-medium mr-1.5">AI Summary</span>
                          ...
                          {hotspot.authorName && (
                            <div className="flex items-center gap-2 mb-3">
                              ...
                              {hotspot.authorVerified && (
                                <span className="text-[10px] px-1.5 py-0.5 rounded bg-blue-500/15 text-blue-400">✓ Verified</span>
                              )}
                              {hotspot.authorFollowers != null && hotspot.authorFollowers > 0 && (
                                <span className="text-[10px] text-slate-600">{hotspot.authorFollowers.toLocaleString()} followers</span>
                              )}
                            </div>
                          )}
                          ...
                          <div className="flex flex-wrap items-center gap-3 text-xs text-slate-600 mb-2">
                            ...
                            {hotspot.danmakuCount != null && hotspot.danmakuCount > 0 && (
                              <span className="flex items-center gap-1" title="Danmaku">
                          ...
                          <div className="flex flex-wrap items-center gap-3 text-[11px] text-slate-600">
                            {hotspot.publishedAt && (
                              <span className="flex items-center gap-1" title={`Published at ${formatDateTime(hotspot.publishedAt)}`}>
                                <Clock className="w-3 h-3" />
                                Published {relativeTime(hotspot.publishedAt)}
                              </span>
                            )}
                            <span className="flex items-center gap-1" title={`Fetched at ${formatDateTime(hotspot.createdAt)}`}>
                              <Activity className="w-3 h-3" />
                              Fetched {relativeTime(hotspot.createdAt)}
                            </span>
                          </div>
          ...
          <div className="space-y-6">
              ...
              <div className="flex gap-3">
                <div className="flex-1 relative">
                  <input
                    ...
                    placeholder="Enter keyword to monitor, e.g. GPT-5, AI Programming, Cursor..."
                        ...
                        <div>
                          ...
                          {keyword._count && keyword._count.hotspots > 0 && (
                            <span className="ml-2 text-xs text-slate-600">
                              {keyword._count.hotspots} hotspots
                            </span>
                          )}
                        </div>
                      </div>
            ...
            {keywords.length === 0 && (
              <div className="text-center py-16 rounded-2xl border border-dashed border-white/10">
                ...
                <p className="text-slate-500">No monitored keywords yet</p>
                <p className="text-sm text-slate-600 mt-1">Add technical hotspot words you want to track</p>
              </div>
            )}
          </div>
        )}
            ...
            <div className="space-y-3">
              {filteredSearchResults.length === 0 && searchResults.length > 0 && (
                <div className="text-center py-12 rounded-2xl border border-dashed border-white/10">
                  <p className="text-slate-500">No results under current filters</p>
                  <p className="text-sm text-slate-600 mt-1">Try adjusting the filter options</p>
                </div>
              )}
